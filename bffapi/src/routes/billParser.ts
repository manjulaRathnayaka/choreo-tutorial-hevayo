...
import { billParserService, accountsService } from '../utils/serviceClient';
 ...
 */
router.post('/parse-bill', upload.single('image'), async (req: FileRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
 ...
 *         description: Server error
 */
router.post('/create-bill-from-image', upload.single('image'), async (req: FileRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No image file provided' });
    }
